...
console.log("--------------------------------------------------------------");
{
  function findPairWithSum(arr, total) {
    ...
    for (let num of arr) {
      ...
      if (set.has(result)) { // если в коллекции есть это число то добавь его и верни num и result 
        return [result, num];
      }
  ...
  // Создали коллекцию -> зашли в цикл по коллекции -> в первой итерации коллекция пустая set(0) -> if не срабатывает так как данного числа (1) нет в коллекции -> добавляем число в коллекцию
  // -> вторая итерация в коллекции уже есть (1) set{0:1} -> if не срабатывает так как данного числа (2) нет в коллекции -> добавляем число в коллекцию
  // -> третья итерация в коллекции уже есть (1,2) set{0:1, 1:2}
}
console.log("--------------------------------------------------------------");
//  Для себя
const testObj = {

...
Promise.reject("a") // 1) создали промис, который сразу отклоняется со значением "а". Имеем промис в состоянии rejected со значением "а"
  .catch((p) => p + "b", (function(){console.log(`я выполнился, ${test = test + 1}`)})()) // 2) Поскольку промис отклонен выполняем первый catch. Внутри значение "a"(передано в параметр "р"). Суммируем "а" + "b" = "ab". Промис перешел в состояние fulfilled с результатом "ab"
  ...
  .finally((p) => p + "e", (function(){test = test + 150})()) // 7) Метод .finally выполняется всегда, независимо от того, был ли промис выполнен или отклонен.Однако важно помнить, что .finally не изменяет результат промиса. Он просто выполняет код внутри себя.
